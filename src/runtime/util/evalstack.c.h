...
void RT_EvalStack_push(const RT_StackEntry_t node)
{
    if (rt_eval_stack.top + 1 >= rt_eval_stack.capacity) {
        ...
        RT_StackEntry_t *new_nodes = (RT_StackEntry_t*) realloc(rt_eval_stack.nodes, rt_eval_stack.capacity * sizeof(RT_StackEntry_t));
        if (!new_nodes) rt_throw("RT_EvalStack_push:" ERR_MSG_REALLOCFAIL);
        rt_eval_stack.nodes = new_nodes;
    }
    ++rt_eval_stack.top;
    rt_eval_stack.nodes[rt_eval_stack.top] = node;
}
...
RT_StackEntry_t RT_EvalStack_pop()
{
    ...
    RT_StackEntry_t node = rt_eval_stack.nodes[rt_eval_stack.top];
    --rt_eval_stack.top;
    if (rt_eval_stack.top == -1) {
        free(rt_eval_stack.nodes);
        rt_eval_stack.nodes = NULL;
...
RT_StackEntry_t RT_EvalStack_top()
{
    ...
    return rt_eval_stack.nodes[rt_eval_stack.top];
}
