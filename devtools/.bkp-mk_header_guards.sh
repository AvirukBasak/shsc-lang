...
for file in $directory/*; do
    ...
    echo "" >> $file
    echo "#include \"runtime/data/Data.h\"" >> $file
    echo "#include \"runtime/operators.h\"" >> $file
    echo "" >> $file
    echo "void rt_op_$(x=`basename $file` && echo ${x%.c.h})(rt_Data_t *lhs, rt_Data_t *rhs) {}" >> $file
    echo "" >> $file
    if [[ $base == *.c.h ]]; then
        echo "#else" >> $file
